...
tf = 100  # last time instance of simulation in [s]
...
b_gm = False     # Simulate Gauss Markov
b_wh = False     # Simulate White Noise
b_rc = False     # Simulate Random Constant

sigma_rw = [1]  # random walk sigma
sigma_gm = [1]  # gauss markov sigma
tau_gm = 1  # gauss markov correlation time
sigma_wn = [1, 1]  # white noise sigma
...
def plots(name, axes, trajectory, psd, allan_var):
    ...
    ax = None
    for i in range(axes):
        ax = plot_1D(f, power[:, i], ax=ax, label=f"Axis {i}", x_label="Frequency [Hz]",
